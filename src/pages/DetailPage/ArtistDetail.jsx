...
function ArtistDetail() {
     ...
     const fetchArtistData = useCallback(async () => {
          try {
               ...
               console.log("🚀 ~ file: ArtistDetail.jsx ~ line 81 ~ fetchArtistData ~ results", results)
                    ...
                    artist && (
                         <div className={cx("detail")}>
                              ...
                              <div className={cx('detail-main')}>
                                   ...
                                   <>
                                        {artist.tracks.map((artist, index) =>
                                             <div key={index.id} className={cx('grid h-[70px] grid-cols-[40px_minmax(500px,_1fr)_60px] grid-flow-cols gap-4 rounded-[6px]  items-center hover:bg-[#d0d0d019]  hover:delay-100 ')}>
                                                  ...
                                                  <div className={cx(" flex items-center  ")}>
                                                       <Image className={'w-[50px]  rounded-[5px] mr-[15px] '} src={baseUrl + artist.image?.path} />
                                                       <Link to={`/detail/track/${artist._id}`} >
                                                            <span className={cx(" w-full overflow-hidden mt-2 text-[16px] font-medium text-left leading-normal hover:underline")}>{artist.name}</span>
                                                       </Link>
                                                  </div>
                                                  <div className={cx()}>
                                                       <span className=" ">
                                                            {millisecondsToMinutesAndSeconds(artist.duration)}
                                                       </span>
                                                  </div>
                                             </div>
                                        ...
                                        <>
                                             <div className=" grid grid-cols-7 grid-flow-row gap-6 ">
                                                  {artist.tracks.map((items) =>
                                                       <div key={items.id} className="w-full p-3 bg-[#252525d1] rounded-[5px] relative"  >
